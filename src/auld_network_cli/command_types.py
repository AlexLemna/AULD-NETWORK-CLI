...
if TYPE_CHECKING:
    # Conditionally allows for imports while type-checking tools scan this file
    # (as part of an IDE or test suite, etc.) but avoids the imports when the
    # file actually runs. This way we can avoid circular imports, etc.
...
class CommandRegistry:
    ...
    def __init__(self) -> None:  # A singleton
        ...
        if not hasattr(self, "_initialized"):
            self._by_mode: Dict[ShellMode, List[Command]] = {
                ShellMode.USER: [],
                ShellMode.ADMIN: [],
            }
    ...
    def _candidates_for_prefix(
        self, mode: ShellMode, input_tokens: Sequence[str]
    ) -> List[Command]:
    ...
    def resolve(self, mode: ShellMode, input_tokens: Sequence[str]) -> Command:
...
def command(
    tokens: Tuple[str, ...] | str, mode: ShellMode, description: str = "(no help given)"
):
